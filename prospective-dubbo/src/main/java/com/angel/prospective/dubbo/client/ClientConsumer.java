...
// 引用远程服务
        ReferenceConfig<VmService> reference = new ReferenceConfig<VmService>(); // 此实例很重，封装了与注册中心的连接以及与提供者的连接，请自行缓存，否则可能造成内存和连接泄漏
        reference.setApplication(application);
        reference.setMonitor("monitor");
        reference.setRegistry(registry); // 多个注册中心可以用setRegistries()
        reference.setInterface(VmService.class);
        //reference.setVersion("1.0.0");

// 和本地bean一样使用xxxService
        VmService vmService = reference.get();
